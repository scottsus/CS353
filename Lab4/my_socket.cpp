 ...
 *
 * @(#)$Id: my_socket.cpp,v 1.1 2023/01/07 07:08:25 william Exp $
 */
...
/**
 * Use this code to create a server listening socket or a client socket.
 *
 ...
 * If a client connects, this function would create a new socket file descriptor (which is a copy of the listening socket file descriptor) for communicating with the client.
 *
 * You should be able to use this function as it.
 *
...
int my_accept(const int listen_socket_fd)
{
    ...
    while (newsockfd == (-1)) {
        ...
        newsockfd = accept(listen_socket_fd, (struct sockaddr *)(&cli_addr), &clilen);
        if (newsockfd == (-1)) {
            if (errno == EINTR) {
