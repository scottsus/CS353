    ...
    while (true)
    {
        ...
        else if (cmd == "traceroute")
        {
            ...
            for (int i = 1; i <= max_ttl; i++)
            {
                ...
                if (event->get_event_type() == 0) // TTLZERO
                {
                    string timediff = timer.stop();
                    cout << i << " - " << next_hop->get_nodeid() << ", " << timediff << endl;
                }
                else if (event->get_event_type() == 1) // PONG
                {
                    string timediff = timer.stop();
                    cout << i << " - " << next_hop->get_nodeid() << ", " << timediff << endl;
                ...
                else if (event->get_event_type() == 2) // Timer lifetime expired
                {
                    cout << i << " - *" << endl;
                }
                timer.get_timer_thread()->join();
