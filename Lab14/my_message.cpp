...
Message::Message(int ttl, int flood_reason, string message_id, string sender_nodeid, string target_nodeid, string origin_nodeid, string origin_start_time, string message_body, int content_len)
{
    this->ok = true;
    this->is_http = false;
    this->should_terminate = false;
...
void Message::set_message_type(string message_type)
{
...
{
    return make_shared<Message>(ttl - 1, flood_reason, message_id, sender_nodeid, target_nodeid, origin_nodeid, origin_start_time, message_body, content_len);
}
...
RDTMessage::RDTMessage(int seq_num, int app_num)
{
...
RDTMessage::RDTMessage(int seq_num, int app_num, string message_body)
{
    this->seq_num = seq_num;
    this->app_num = app_num;
...
int RDTMessage::get_content_len()
{
    return content_len;
}

string RDTMessage::get_message_body()
{
    return message_body;
}