...
void await_p2p_request(string nodeid, int neighbor_socketfd, shared_ptr<Connection> conn, vector<shared_ptr<Connection>> *conns)
{
    ...
    map<string, shared_ptr<RDTState>> sessions{};
    ...
    if (!is_duplicate_conn)
    {
        ...
        while (true)
        {
            ...
            if (message->get_message_type() == "RDT")
            {
                cout << "RDT message received from " << message->get_origin_nodeid() << endl;

                shared_ptr<RDTMessage> rdt_data_packet = dynamic_pointer_cast<RDTMessage>(message);
                string message_received = "";
                int seq_num = rdt_data_packet->get_seq_num();
                int app_num = rdt_data_packet->get_app_num();
                int content_len = rdt_data_packet->get_content_len();
                string message_body = rdt_data_packet->get_message_body();

                string src_nodeid = message->get_origin_nodeid();
                shared_ptr<RDTState> session = sessions[src_nodeid];
                if (session == NULL)
                {
                    session = make_shared<RDTState>(src_nodeid, seq_num, app_num, content_len, message_body);
                ...
                shared_ptr<RDTMessage> rdt_ack_message = make_shared<RDTMessage>(RDTMessage(seq_num, app_num));
                next_hop_conn->add_message_to_queue(rdt_ack_message);

                while (true)
                {
                    if (rdt_data_packet->get_seq_num() == seq_num)
                    {
                        string data = rdt_data_packet->get_message_body();
                        if (data != "\n")
                    ...
                    shared_ptr<Message> message = await_message_from(neighbor_socketfd, conn);
                    rdt_data_packet = dynamic_pointer_cast<RDTMessage>(message);
                }

                // deliver msg_rcvd to app or print msg_rcvd
                sessions.erase(src_nodeid);
            }

            else if (message->get_target_nodeid() != "") // type UCASTAPP
            {
                log_UCASTAPP("r", message);
                string message_body = message->get_message_body();
                ...
                if (message->get_ttl() <= 0 && target_nodeid != nodeid)
                {
                    ...
                    Message ucastapp_zero = Message(max_ttl, nodeid, message->get_origin_nodeid(), 1, zero_body.length(), zero_body);
                    next_hop_conn->add_message_to_queue(make_shared<Message>(ucastapp_zero));
                ...
                if (target_nodeid == nodeid)
                {
                    ...
                    else if (message_type == "PING")
                    {
                        ...
                        Message ucastapp_pong(max_ttl, nodeid, src_nodeid, 1, pong_body.length(), pong_body);
                        next_hop_conn->add_message_to_queue(make_shared<Message>(ucastapp_pong));
                    }
                    continue;
                }
...
void write_RDT(shared_ptr<Connection> conn, shared_ptr<Message> message)
{
    string h1 = "353UDT/1.0 RDTDATA\r\n";
    // string h2 = "Seq-Number: " +
    // string h3 = "RDT-App-Number: " +
    string h4 = "RDT-Content-Length: " + to_string(message->get_content_len()) + "\r\n";
    string h5 = "\r\n";
    string h6 = message->get_message_body();
    ...
    better_write_header(next_socketfd, h4.c_str(), h4.length());
    better_write_header(next_socketfd, h5.c_str(), h5.length());
    better_write(next_socketfd, h6.c_str(), h6.length());
