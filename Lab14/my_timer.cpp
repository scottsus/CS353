...
tuple<double, string> Timer::stop()
{
    cancelled = true;
    gettimeofday(&now_time, NULL);
    double diff_num = timestamp_diff_in_seconds(&start_time, &now_time);
    string diff_str = str_timestamp_diff_in_seconds(&start_time, &now_time);

    return make_tuple(diff_num, diff_str);
}
...
void countdown(int expiration_time)
{
    ...
    mut.lock();
    timer_q.push(make_shared<Event>("TIMEOUT"));
    timer_cv.notify_one();
    mut.unlock();
}
