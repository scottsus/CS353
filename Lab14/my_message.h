...
public:
    ...
    void set_message_type(string message_type);

    shared_ptr<Message> decr_ttl_update_sender(string sender_nodeid);
    string to_string();
};

class RDTMessage : public Message
{
public:
    RDTMessage(int seq_num, int app_num, string message_body);
    RDTMessage(int seq_num, int app_num);
